...
const distanceToFourth = currentLocation.distanceTo(fourthCheckpoint);
    ...
    nearestDistance.innerHTML = `Distance to first point: ${Math.round(distanceToSecond)}m`;
...
if (distanceToThird < 100) {
  console.log("In 100 meters, turn left");
  const audio100left2 = document.getElementById('audio100left'); // Assume this is the ID for the third checkpoint left turn audio
  audio100left2.play();
}

// Check proximity to the fourth checkpoint and play corresponding audio
if (distanceToFourth < 100) {
  console.log("In 100 meters, turn right");
  const audio100right2 = document.getElementById('audio100right'); // Assume this is the ID for the fourth checkpoint right turn audio
  audio100right2.play();
}
